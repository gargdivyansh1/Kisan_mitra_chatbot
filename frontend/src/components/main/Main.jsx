...
const Main = () => {
  ...
  useEffect(() => {
    const fetchConversationHistory = async () => {
      if (activeSessionId) {
        try {
          const history = await conversation_of_current_session(activeSessionId);
...
>>>>>>> f55c7832e09f6e14d96f2d926f4d3dc7313a1036
    );
  };
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await axios.post("http://127.0.0.1:8000/farmer_query/chat", {
        user_id: user_id,
        message: input,
        session_id: req !== "नया_सत्र" ? `${req}` : `${input}`
      });
      setValue(val + 1);
      const botMessage = {
        type: "bot",
  ...
  return (
    <div className="flex min-h-screen bg-gradient-to-b from-amber-50 to-emerald-50">
      <div className={`${sidebarOpen ? "w-64" : "w-0"} transition-all duration-300 bg-gradient-to-b from-emerald-800 to-emerald-900 text-white overflow-hidden flex flex-col`}>
        <div className="p-4 flex items-center justify-between border-b border-emerald-700">
          <h2 className="text-xl font-bold">सत्र</h2>
          <button onClick={() => setSidebarOpen(false)} className="p-1 rounded hover:bg-emerald-700">✕</button>
        </div>
        <div className="flex-1 overflow-y-auto p-2">
          <button
            onClick={createNewSession}
      ...
      <div className="flex-1 min-h-screen flex flex-col">
        <div className="flex items-center justify-between px-4 py-3 sm:px-6 bg-gradient-to-r from-emerald-800 to-amber-800 text-white shadow-md">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <img src={assets.user} alt="user" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-amber-200 shadow-md" />
          </div>
        </div>
